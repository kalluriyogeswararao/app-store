...
class AppStore extends Component {
  state = {
    ...
    searchInput: '',
    appDisplayDetails: appsList,
  }

  onChangeItem = event => {
    const searchResults = appsList.filter(app =>
      app.appName.toLowerCase().includes(event.target.value.toLowerCase()),
    )
    this.setState({
      appDisplayDetails: searchResults,
  ...
  getfilteredProjects = () => {
    const {appDisplayDetails, activeTabid} = this.state
    const filteredProjects = appDisplayDetails.filter(
    ...
    return (
      <div className="bg-container">
        ...
        <ul className="app-items">
          {filteredProjects.map(eachAppDetails => (
